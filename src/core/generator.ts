...
<body>
  ...
  private async generateReadme(outputPath: string): Promise<void> {
    ...
    const content = this.renderTemplate(tplName, {
      ...
      securitySchemes: this.parser.getSpec().components?.securitySchemes || null,
    });
  ...
  private generateReadmeContent(data: any): string {
    ...
    if (data.securitySchemes && Object.keys(data.securitySchemes).length > 0) {
      content += `\n## Authentication\n\n`;
      for (const [name, scheme] of Object.entries(data.securitySchemes) as [string, any][]) {
        content += `### ${name}\n\n`;
        if (scheme.type === 'http') {
          content += `- **Type:** HTTP Authentication\n`;
          content += `- **Scheme:** \`${scheme.scheme}\``;
          if (scheme.bearerFormat) content += ` (${scheme.bearerFormat})`;
          content += '\n\n';
        } else if (scheme.type === 'apiKey') {
          content += `- **Type:** API Key\n`;
          content += `- **Location:** ${scheme.in} (\`${scheme.name}\`)\n\n`;
        } else if (scheme.type === 'oauth2') {
          content += `- **Type:** OAuth 2.0\n`;
          content += `- **Flows:** ${Object.keys(scheme.flows || {}).join(', ')}\n\n`;
        } else if (scheme.type === 'openIdConnect') {
          content += `- **Type:** OpenID Connect\n`;
          content += `- **Connect URL:** ${scheme.openIdConnectUrl}\n\n`;
        } else {
